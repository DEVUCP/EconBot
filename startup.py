...
def ValidSettings() -> bool:
    ...
    for value in settings["game"]["gambling"].values():
        gambling_valid = IsValidGambling(value)
        if not gambling_valid:
            print_colored(f"[!! INVALID GAMBLING !!]", "red")
            print_colored("EXITING SETTINGS . . .", "yellow")
            return False
    
    print_colored("[ VALID GAMBLING ]", "green")
...
def IsValidGambling(value : any) -> bool:
    '''Checks if the gambling setting is valid.'''
...
def GenerateHelpCommand():
    ...
    if not constants.ENABLE_BJ:
        del constants.COMMANDS["gambling"]["blackjack"]

    if not constants.ENABLE_SLOTS:
        del constants.COMMANDS["gambling"]["slots"]

    if not constants.ENABLE_ROULETTE:
        del constants.COMMANDS["gambling"]["roulette"]
    
    if not constants.ENABLE_BJ and not constants.ENABLE_SLOTS and not constants.ENABLE_ROULETTE:
        del constants.COMMANDS["gambling"]